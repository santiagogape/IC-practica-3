...
#define SYNCH_WORD 0x47
// NOTA: Ajustar estas variables 
const uint8_t localAddress = 0x48;     // Dirección de este dispositivo
uint8_t destination = 0x49;            // Dirección de destino, 0xFF es la dirección de broadcast
...
void setup() 
{
  ...
  LoRa.setSyncWord(SYNCH_WORD);         // Palabra de sincronización privada por defecto para SX127X 
...
void onReceive(int packetSize) 
{
  ...
  if (recipient != localAddress ) {
    Serial.println("Receiving error: This message is not exactly for me.");
    return;
  }
